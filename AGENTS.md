...
- `FluxForm.Core`
  ...
  - `Dependencies/`：`IDependencyManager`、`DependencyManager`（管理 ffmpeg 路径；开发时自动下载，发布时可通过 MSBuild 可选内置）
...
# 发布 WPF 标准安装包（v0.1.1 普通用户入口，需安装 Inno Setup 6）
./scripts/publish-installer.ps1

# 如需内置 FFmpeg，优先准备 tools-cache/ffmpeg-release-essentials.zip 后显式启用
...
# 如需额外构建安装包
./scripts/release-check.ps1 -BuildInstaller
```

如需验证 WPF 主窗口能启动出来，运行：
...
- `global.json` 固定 .NET 9 SDK 选择策略，避免不同机器使用差异过大的 SDK。
- `Directory.Build.props` 启用 NuGet lock file；新增或升级 NuGet 包后必须运行 `dotnet restore .\FluxForm.sln --use-lock-file` 并提交对应 `packages.lock.json`。发布脚本使用独立的临时 lock path，避免 RID 发布修改项目根目录的锁文件。
- `.editorconfig` 与 `.gitattributes` 统一 UTF-8、缩进和 CRLF 行尾策略，避免中文文档和 XAML/PowerShell 文件产生无关 diff。
...
5. 如果改动涉及 `FluxForm.WPF/MainWindow.xaml` 或 `FluxForm.WPF/ViewModels/MainViewModel.cs`，必须至少手动走一遍黄金路径：添加输入 → 配置参数 → 开始任务 / 停止任务 → 检查日志与状态反馈。
...
- ffmpeg：普通发布默认不联网下载 FFmpeg，避免 `FFmpegForPublish` 长时间阻塞或因网络留下损坏压缩包。若 `tools-cache/ffmpeg-release-essentials.zip` 存在，或发布脚本传入 `-BundleFFmpeg`，`build/FFmpeg.targets` 会解压到发布目录的 `tools/ffmpeg/` 下；若确需发布时联网下载，传入 `-DownloadFFmpeg`。发布目录中若已存在 `tools/ffmpeg/ffmpeg.exe`，打包步骤会跳过。开发调试时若本地不存在，仍由 `DependencyManager` 自动下载到 `tools/ffmpeg/`。
- LibreOffice：文档转换时优先在系统常见目录查找，未找到则提示用户安装。未启用自动下载（安装包过大）。
- Inno Setup：v0.1.1 的普通桌面分发产物为 `FluxFormSetup-0.1.1.exe`，通过 `scripts/publish-installer.ps1` 调用 Inno Setup 6 生成；直接运行 `publish/wpf/FluxForm.WPF.exe` 仍作为开发验证路径保留。
