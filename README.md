...
- **文档**：PDF、DOCX、XLSX、PPTX、TXT、HTML 等格式互转（需要安装 LibreOffice）

## 运行环境

- Windows 10/11 x64
- 正式发布产物采用自包含发布，最终用户无需额外安装 .NET 9 运行时
...
桌面版仍可直接进入 `publish/wpf`，运行 `FluxForm.WPF.exe`。如果发布时启用了 FFmpeg 打包，请保留同级 `tools/ffmpeg/` 目录不变。

命令行版进入 `publish/cli`，运行 `FluxForm.CLI.exe`。如果发布时启用了 FFmpeg 打包，请保留同级 `tools/ffmpeg/` 目录不变：
...
- **FFmpeg**：开发调试时，如果本地不存在，FluxForm 会自动下载并解压到 `tools/ffmpeg/`。正式 publish 默认不联网下载 FFmpeg，以保证发布速度和稳定性；若 `tools-cache/ffmpeg-release-essentials.zip` 存在，或发布脚本显式传入 `-BundleFFmpeg`，MSBuild 会将 FFmpeg 放入发布目录下的 `tools/ffmpeg/`。如果确实需要发布时联网下载，使用 `-DownloadFFmpeg`。安装包发布脚本会显式启用 FFmpeg 打包。
...
如需生成 WPF 标准安装包，需先安装 Inno Setup 6，然后运行：

```powershell
./scripts/publish-installer.ps1
```

如果 Inno Setup 未安装到常见目录，可以显式传入编译器路径：

```powershell
./scripts/publish-installer.ps1 -InnoSetupCompilerPath "C:\Program Files (x86)\Inno Setup 6\ISCC.exe"
```
